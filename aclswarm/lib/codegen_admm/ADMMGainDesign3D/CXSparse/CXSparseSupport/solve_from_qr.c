...
#if defined(_MSC_VER) && defined(CS_COMPLEX)
    return hypot(creal(in), cimag(in));
#else
    return cabs(in);
#endif
}
    ...
    *tol = scale * complexAbs(mat->x[0]);
    ...
    for (col = 0; col < minSize; col++) {
        ...
        for (rowOffset = 0; rowOffset < nnzThisCol; rowOffset++) {
            ...
            if (row == col) {
                if(complexAbs(mat->x[p[col] + rowOffset]) > *tol){
                    rank = rank+1;
                    break;
                }else {
                    break;
...
void solve_from_qr_di(cs_din* N, cs_dis* S, double* b, int m, int n) {
    double* x = cs_di_calloc(S ? S->m2 : 1, sizeof(double)); /* get workspace */
    if (m >= n) {
...
void solve_from_qr_ci(cs_cin* N, cs_cis* S, cs_complex_t* b, int m, int n) {
    cs_complex_t* x = cs_ci_calloc(S ? S->m2 : 1, sizeof(cs_complex_t)); /* get workspace */
    if (m >= n) {
